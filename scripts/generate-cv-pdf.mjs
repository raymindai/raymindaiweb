...
const content = {
  ...
  ko: {
    ...
    heroQuote: 'AI 중심의 프로덕트 및 경험 리더. 영국, 한국, 사우디아라비아를 넘나들며 20년 이상 국제적 리더십을 쌓아왔다. 엔터프라이즈 AI 플랫폼, 공간 컴퓨팅, 글로벌 디지털 생태계를 위한 디자인 및 프로덕트 조직을 구축하고 확장했다. AI를 프로덕트 전략, 멀티모달 경험 설계, 크로스펑셔널 워크플로우에 걸쳐 운영화하는 데 특화되어 있다.',
    ...
    humainHighlights: [
      '엔터프라이즈 AI 이니셔티브 전반의 플랫폼 레벨 프로덕트 비전 정의',
      '확장성과 일관성을 보장하는 거버넌스 및 리뷰 프레임워크 수립',
      '웨어러블 및 앰비언트 인터랙션 모델을 포함한 멀티모달 AI 확장 주도',
      '리서치, 디자인, 문서화 전반에 AI 지원 워크플로우 내재화',
      '프로덕트, 엔지니어링, 리더십 간 크로스펑셔널 운영 체계 구축',
    ],
    devDesc: '쿠키런 IP 기반 공간 컴퓨팅 제품을 0에서 1까지 주도했다. 팀을 만들고, Meta 파트너십을 확보하고, Quest Store에 출시했다.',
    devHighlights: [
      'XR 비네이티브 조직에서 공간 컴퓨팅 역량 및 제품 파이프라인 구축',
      'Meta와의 전략적 플랫폼 파트너십 확보, Quest Store 전체 라이프사이클 런칭',
      '프로덕트, 엔지니어링, 아트, 마케팅 크로스펑셔널 팀 총괄',
      '플랫폼 프로모션에 피처된 프로덕션 레디 몰입형 경험 제공',
    ],
    kakaoDesc: '카카오 최초의 유럽 법인을 처음부터 설립하고 이끌었다. 카카오톡과 카카오프렌즈 IP의 유럽 확장을 추진했다.',
    kakaoHighlights: [
      '유럽 운영 기반 및 현지화 전략 구축',
      '유럽 주요 시장에서 라이선싱 및 리테일 파트너십 런칭',
      'IMG와의 전략적 제휴로 IP 글로벌 풋프린트 확장',
      '주요 글로벌 트레이드 이벤트 및 엔터프라이즈 협상 대표',
    ],
    aramcoDesc: '세계 최대 IPO(2019)를 앞두고 아람코 전사 디지털 생태계에 적용된 엔터프라이즈 UX 표준을 정의했다.',
    aramcoHighlights: [
      '전사 디지털 생태계 전반의 엔터프라이즈 UX 표준 정의',
      '산업 교육 및 커뮤니케이션 지원 몰입형 VR 애플리케이션 구현',
      '국가 정책 전환을 지원하는 디지털 인프라 설계',
      'IPO 준비 단계의 기업 디지털 현대화 기여',
    ],
    akqaDesc: 'Nike(NRC, NTC), Volvo(글로벌 웹), Barclays(모기지), Virgin Atlantic(온라인 예약) 전략적 UX 프로그램 주도. AI 기반 추천 경험 및 데이터 기반 개인화 시스템 구현.',
    razorfishDesc: '금융 및 소비자 브랜드의 다중 시장 디지털 프로그램에서 UX 딜리버리를 주도. 엔터프라이즈 규모의 UX 팀 관리와 크로스펑셔널 협업 이끔.',
    expertise1: { title: '프로덕트 & 전략', desc: 'AI 네이티브 프로덕트 전략, 엔터프라이즈 AI 플랫폼 개발, 제로투원 프로덕트 인큐베이션' },
    expertise2: { title: '경험 & 디자인', desc: '인간-AI 및 멀티모달 인터랙션, 공간 컴퓨팅 및 웨어러블, 디자인 거버넌스 및 시스템 아키텍처' },
    expertise3: { title: '리더십 & 스케일', desc: '크로스펑셔널 경영 리더십, 국가 규모 디지털 전환, 해외 시장 확장' },
    langText: '<strong>한국어</strong> 모국어 &nbsp;&nbsp;&nbsp; <strong>영어</strong> 비즈니스 레벨',
  },
};
